...
def setPcb():
    ...
    while(i < n):
        print("-------------------------------------")
        pID = chr(65+i)
        print("please enter info of %s process "%pID)
        ...
        pcb.append([pID, arriveTime, serviceTime, 0, 0, 0, 0]) 
...
def SJF():
    ...
    for i in range(n):
        print("进程 %s    完成时间: %d 周转时间: %d 带权周转时间: %.2f" \
              % (sjf_pcb[i][0], int(sjf_pcb[i][4]), int(sjf_pcb[i][5]), float(sjf_pcb[i][6])))
        i += 1
    print("本次调度的平均周转时间为： %.2f" % float(AzzTime))
    print("本次调度的平均带权周转时间为： %.2f" % float(AdqzzTime))
...
if __name__ == "__main__":
    ...
    m=1
    while(m==1):
        option=int(input("choose your method to start( 1.FCFS or 2.SJF ):"))
        if(option==1):
            FCFS()
        if(option==2):
            SJF()