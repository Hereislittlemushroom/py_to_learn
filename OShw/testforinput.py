...
def outputPcb(pcb):
    ...
    for i in range(len(pcb)):
        print("进程%s\t完成时间: %d\t周转时间: %d\t带权周转时间: %.2f\t优先级: %d"
              % (
                 pcb[i]['ID'], pcb[i]['completeTime'],pcb[i]['RTime'], 
                 pcb[i]['WRTime'], pcb[i]['prior']
                )
             )
        i += 1
    print("该算法调度的平均周转时间为:\t%.2f" % float(aRTime))
    print("该算法调度的平均带权周转时间为:\t%.2f" % float(aWRTime))
